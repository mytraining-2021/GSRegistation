FROM openjdk:17
WORKDIR /app
COPY target/GSRegistration.jar GSRegistration.jar
...
EXPOSE 8080
ENTRYPOINT ["java", "-javaagent:/applicationinsights-agent-3.2.9.jar", "-Dapplicationinsights.connection.string=${APPLICATIONINSIGHTS_CONNECTION_STRING}", "-jar", "GSRegistration.jar"]
